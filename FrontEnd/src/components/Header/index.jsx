      ...
      <ul className="flex items-center gap-9">
        <li>
          <Link to={`/homepage?matricule=${matricule}&prenom=${prenom}`} className="cursor-pointer">
            <Text size="sm" as="p" className="text-sm tracking-[-0.50px+] !text-black-900">Home</Text>
          </Link>
        </li>
        <li>
        <Link to={`/test?matricule=${matricule}&prenom=${prenom}`} className="cursor-pointer">
            <Text size="sm" as="p" className="text-sm !font-normal tracking-[-0.50px] !text-black-900">Passer le Test</Text>
          </Link>
        </li>
        <li>
        <Link to={`/about?matricule=${matricule}&prenom=${prenom}`} className="cursor-pointer">
            <Text size="sm" as="p" className="text-sm !font-normal tracking-[-0.50px] !text-black-900">About us</Text>
          </Link>
        </li>
      </ul>
      
      {/* Bouton à la fin */}
      <Button color="purple_400_01" size="sm" shape="round" className="font-medium h-8 w-13 text-sm"
